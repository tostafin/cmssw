...
private:
  ...
  double findTimeOffset(const TH2F*, const double, const double);
  double findGaussianMean(const std::unique_ptr<TH1D>& tProjection, const double estimatedMean);

  std::unordered_map<PlaneKey, std::pair<unsigned int, double>, PlaneKeyHash> lsAndTimeOffset_;
