...
void PPSTimingCalibrationPCLWorker::dqmBeginRun(const edm::Run& iRun,
                                                ...
                                                TimingCalibrationData& iData) const {
std::vector<CTPPSDiamondDetId> detIds;
  ...
  iData.doublePeakCorrection.extractLsAndTimeOffset(tVsLsFilename_, iRun.run(), detIds);
}
...
void PPSTimingCalibrationPCLWorker::bookHistograms(DQMStore::IBooker& iBooker,
  ...
  for (auto it = geom.beginSensor(); it != geom.endSensor(); ++it) {
    if (CTPPSDiamondDetId::check(it->first)) {
      ...
      iData.leadingTime[channelId] =
          iBooker.book1D("t_" + channelName, channelName + ";t (ns);Entries", 1200, -60.0, 60.0);
      iData.toT[channelId] = iBooker.book1D("tot_" + channelName, channelName + ";ToT (ns);Entries", 160, -20.0, 20.0);
      iData.leadingTimeVsToT[channelId] =
          iBooker.book2D("tvstot_" + channelName, channelName + ";ToT (ns);t (ns)", 240, 0.0, 60.0, 450, -20.0, 25.0);
      if (iData.leadingTimeVsLs.count(planeKey) == 0) {
        detId.planeName(planeName);
        iData.leadingTimeVsLs[planeKey] =
            iBooker.book2D("tvsls_" + planeName, planeName + ";LS;t (ns)", 3000, 1, 3000, 500, 0, 20);
      }
    }
  }
}
...
void PPSTimingCalibrationPCLWorker::dqmAnalyze(const edm::Event& iEvent,
                                               const edm::EventSetup& iSetup,
                                               const TimingCalibrationData& iData) const {
  ...
  for (const auto& dsRechits : *dsvRechits) {
    ...
    const uint32_t channelId{detId.rawId()};
    if (iData.leadingTimeVsToT.count(channelId) == 0) {
    ...
    for (const auto& rechit : dsRechits) {
      ...
      if (rechit.time() != 0.0 && rechit.toT() >= 0.0) {
        const double correctedLeadingTime{
            iData.doublePeakCorrection.getCorrectedLeadingTime(rechit.time(), ls, planeKey)};
        iData.leadingTime.at(channelId)->Fill(correctedLeadingTime);
        iData.toT.at(channelId)->Fill(rechit.toT());
        iData.leadingTimeVsToT.at(channelId)->Fill(rechit.toT(), correctedLeadingTime);
        iData.leadingTimeVsLs.at(planeKey)->Fill(ls, correctedLeadingTime);
      }
    }
  }
}
