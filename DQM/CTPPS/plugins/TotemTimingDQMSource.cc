...
//   trackDistribution = ibooker.book1D( "tracks", title+" tracks;x (mm)", 19.*INV_DISPLAY_RESOLUTION_FOR_HITS_MM, -1, 18 );    //TODO needs tracks
  ...
  stripTomography210 = ibooker.book2D( "tomography 210", title+" tomography with strips 210 (all planes);x + 25*plane(mm);y (mm)", 200, -50, 150, 200, -50, 150 );
  stripTomography220 = ibooker.book2D( "tomography 220", title+" tomography with strips 220 (all planes);x + 25*plane(mm);y (mm)", 200, -50, 150, 200, -50, 150 );
...
//   hitProfile = ibooker.book1D( "hit profile", title+" hit profile;x (mm)", 19.*INV_DISPLAY_RESOLUTION_FOR_HITS_MM, -1, 18 );    //TODO needs RecHits
  ...
  stripTomography210 = ibooker.book2D( "tomography 210", title+" tomography with strips 210;x (mm);y (mm)", 60, -30, 30, 60, -30, 30 );
  stripTomography220 = ibooker.book2D( "tomography 220", title+" tomography with strips 220;x (mm);y (mm)", 60, -30, 30, 60, -30, 30 );
}
  ...
  for ( const auto& digis : *timingDigis ) {
    ...
    for ( const auto& digi : digis ) {
          ...
          for ( const auto& ds : *stripTracks ) {
            ...
            for ( const auto& striplt : ds ) {
              if ( striplt.isValid() && stripId.arm() == detId.arm() )
              {
                ...
                if ( stripId.rp() - detId.rp() == ( TOTEM_STRIP_MAX_RP_ID - TOTEM_TIMING_MAX_RP_ID ) )
                {
            ...
            for ( const auto& striplt : ds ) {
              if ( striplt.isValid() && stripId.arm() == detId.arm() )
              {
                ...
                if ( stripId.rp() - detId.rp() == ( TOTEM_STRIP_MAX_RP_ID - TOTEM_TIMING_MAX_RP_ID ) )
                {
                  ...
                  double y = striplt.getY0() - rp_y;
                  if ( stripId.station() == TOTEM_STATION_210 )
                    planePlots_[detId_plane].stripTomography210->Fill( x , y );
                  else if ( stripId.station() == TOTEM_STATION_220 )
                    planePlots_[detId_plane].stripTomography220->Fill( x , y );
                }
              }
            }
          }
      ...
      if ( channelPlots_.find( detId ) != channelPlots_.end() )
      {
        ...
        channelPlots_[ detId ].timestampA->Fill( digi.getTimestampA() );
        channelPlots_[ detId ].timestampB->Fill( digi.getTimestampB() );
        channelPlots_[ detId ].timestampDiff->Fill( digi.getTimestampA() - digi.getTimestampB() );
            ...
            for ( const auto& striplt : ds ) {
              if ( striplt.isValid() && stripId.arm() == detId.arm() )
              {
                ...
                if ( stripId.rp() - detId.rp() == ( TOTEM_STRIP_MAX_RP_ID - TOTEM_TIMING_MAX_RP_ID ) )
                {
                  ...
                  double y = striplt.getY0() - rp_y;
                  if ( stripId.station() == TOTEM_STATION_210 )
                    channelPlots_[ detId ].stripTomography210->Fill( x , y );
                  else if ( stripId.station() == TOTEM_STATION_220 )
                    channelPlots_[ detId ].stripTomography220->Fill( x , y );
                }
              }
            }
          }
