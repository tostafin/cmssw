...
namespace cond {
  ...
  namespace impl {
    ...
    LHCInfoPerFill::FillType from_string(const std::string& attributeValue) {
      return from_string_impl<LHCInfoPerFill::FillType, &LHCInfoPerFillPopConImpl::fillTypeFromString>(attributeValue, LHCInfoPerFill::UNKNOWN);
    }

    template <>
    LHCInfoPerFill::ParticleType from_string(const std::string& attributeValue) {
      return from_string_impl<LHCInfoPerFill::ParticleType, &LHCInfoPerFillPopConImpl::particleTypeFromString>(attributeValue, LHCInfoPerFill::NONE);
    }
...
namespace LHCInfoPerFillImpl {
  bool comparePayloads(const LHCInfoPerFill& rhs, const LHCInfoPerFill& lhs) {
    if (rhs.fillNumber() != lhs.fillNumber() ||
        rhs.delivLumi() != lhs.delivLumi() ||
        rhs.recLumi() != lhs.recLumi() ||
        rhs.instLumi() != lhs.instLumi() ||
        rhs.instLumiError() != lhs.instLumiError() ||
        rhs.lhcState() != rhs.lhcState() ||
        rhs.lhcComment() != rhs.lhcComment() ||
