...
{
     this->clear();
     std::cout << "LHC info label: " << lhcInfoLabel_ << std::endl;
     std::cout << "Optics   label: " << opticsLabel_ << std::endl;
     ...
     iSetup.get<CTPPSOpticsRcd>().get(opticsLabel_, opticalFunctions_);
     ...
     for (HepMC::GenEvent::particle_const_iterator eventParticle = evt->particles_begin();
                     eventParticle != evt->particles_end(); ++eventParticle) {

          if (!((*eventParticle)->status() == 1 && (*eventParticle)->pdg_id() == 2212)) continue;

          if (!(fabs((*eventParticle)->momentum().eta()) > etaCut_ &&
                fabs((*eventParticle)->momentum().pz()) > momentumCut_)) continue; // discard protons outside kinematic acceptance
...
bool OpticalFunctionsTransport::transportProton(const HepMC::GenParticle* in_trk)
{
     ...
     {
         arm = 0;
         beamMomentum = (beamParameters_)?(*beamParameters_).getBeamMom45():beamMomentum_;
         xangle = (beamParameters_)?(*beamParameters_).getHalfXangleX45():fCrossingAngle_45;
         ...
         arm = 1;
         beamMomentum = (beamParameters_)?(*beamParameters_).getBeamMom56():beamMomentum_;
         xangle = (beamParameters_)?(*beamParameters_).getHalfXangleX56():fCrossingAngle_56;
  ...
  // calculate kinematics for optics parametrisation
     ...
     if (verbosity_) {
         LogDebug("OpticalFunctionsTransport") << "simu: xi = " << xi << ", th_x_phys = " << th_x_phys << ", th_y_phys = " << th_y_phys
  ...
  // transport the proton into each pot/scoring plane
     for (const auto &ofp : (*opticalFunctions_)) {
          ...
          if (verbosity_) {
              LogDebug("OpticalFunctionsTransport") << "    proton transported: a_x = " << a_x << " rad, a_y = " << a_y << " rad, b_x = " << b_x
