...
/************************************************
    History plots
*************************************************/
template <class PayloadType, CTPPSOptics::RP rp, CTPPSOptics::Function fun>
class OpticalFunction_History : public cond::payloadInspector::HistoryPlot<PayloadType, float> {
public:
  OpticalFunction_History()
      : cond::payloadInspector::HistoryPlot<PayloadType, float>( "Angle: , " +
            CTPPSOptics::getStringFromRPEnum(rp) + ", " + CTPPSOptics::getFunctionName(fun) +
                " Xi = 0.1 vs. Runs",
            CTPPSOptics::getStringFromRPEnum(rp) + " " + CTPPSOptics::getFunctionName(fun)) {
                cond::payloadInspector::PlotBase::addInputParam("Angle");
                cond::payloadInspector::PlotBase::addInputParam("Xi");
            }

  float getFromPayload(PayloadType& payload) override {

    auto paramValues = cond::payloadInspector::PlotBase::inputParamValues();
    //auto theTag = cond::payloadInspector::PlotBase::getTag<0>();
    auto angle = paramValues.find("Angle")->second;
    auto Xi = paramValues.find("Xi")->second;

    for(auto& map_item: payload){
        if(map_item.first == angle){
            for(auto& pot: map_item.second){
                if(CTPPSOptics::decodeRP(pot.first) == rp){
                    return pot.second.getFcnValues()[fun][std::atoi(Xi.c_str())*1000];
            }
          }
        }
      }  
...
template <class PayloadType, cond::payloadInspector::IOVMultiplicity nIOVs, int ntags, CTPPSOptics::Function fun>
...
public:
  OpticalFunctionRPs_History()
      : cond::payloadInspector::PlotImage<PayloadType, nIOVs, ntags> ("Angle: , " +
  ...
  bool fill() override {
    ...
    for(auto IOV : theIOVs){
        ...
        file << "tag: "<< cond::payloadInspector::PlotBase::getTag<0>().name << "IOV: " << std::get<0>(IOV) <<"\n";
        for(auto& map_item: *m_payload){
            file << "angle " << map_item.first<<"\n";
            for(auto& pot: map_item.second){
                for(auto& xi: pot.second.getXiValues()){
                    file<<xi<<"\t";
                }
            }
            file<<"\n\n";
            ...
            if(map_item.first == angle){
                for(auto& pot: map_item.second){
                    ...
                    {
                    case 3:
                        rp3.push_back(std::make_pair(static_cast<long long>(std::get<0>(IOV)), pot.second.getFcnValues()[fun][std::atoi(Xi.c_str())*1000]));
                        break;
                    case 23:
                        rp23.push_back(std::make_pair(static_cast<long long>(std::get<0>(IOV)), pot.second.getFcnValues()[fun][std::atoi(Xi.c_str())*1000]));
                        break;
                    case 103:
                        rp103.push_back(std::make_pair(static_cast<long long>(std::get<0>(IOV)), pot.second.getFcnValues()[fun][std::atoi(Xi.c_str())*1000]));                        
                        break;
                    case 123:
                        rp123.push_back(std::make_pair(static_cast<long long>(std::get<0>(IOV)), pot.second.getFcnValues()[fun][std::atoi(Xi.c_str())*1000]));                        
                        break;
    ...
    mg->SetTitle(("Angle: , " + CTPPSOptics::getFunctionName(fun) +", Xi = 0.001 vs Runs; Run no;" + CTPPSOptics::getFunctionName(fun)).c_str()); 
    Double_t *rp_x, *rp_y;
