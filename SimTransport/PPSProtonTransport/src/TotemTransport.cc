...
TotemTransport::TotemTransport(const edm::ParameterSet& iConfig)
      ...
      m_beampipe_aperture_radius(iConfig.getParameter<double>("BeampipeApertureRadius"))
 {
  ...
  if (fPPSRegionStart_56>0) fPPSRegionStart_56*=-1; // make sure sector 56 has negative position, as TOTEM convention
...
//
  ...
  // (bApplyZShift) -- The TOTEM parameterization requires the shift to z=0
     ...
     if (verbosity_)
         LogDebug("TotemTransport") << "TotemTransport:filterPPS: barcode = "
                                                    << line << " x=  " << x1_ctpps << " y= " << y1_ctpps;
