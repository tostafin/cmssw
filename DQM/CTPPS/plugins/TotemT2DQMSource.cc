...
private:
  ...
  double HPTDC_BIN_WIDTH_NS_ = 25. / 1024;
  MonitorElement* HPTDCErrorFlags_2D_ = nullptr;

  unsigned int nbinsx_, nbinsy_;
...
TotemT2DQMSource::PlanePlots::PlanePlots(DQMStore::IBooker& ibooker,
  ...
  digisMultiplicity = ibooker.book2DD("digis multiplicity",
                                      ...
                                      0.,
                                      nbinsy);
  rechitMultiplicity = ibooker.book2DD(
      "rechits multiplicity", title + " rechits multiplicity;x;y", nbinsx, 0., nbinsx, nbinsy, 0., nbinsy);
}
...
void TotemT2DQMSource::bookErrorFlagsHistogram(DQMStore::IBooker& ibooker) {
  HPTDCErrorFlags_2D_ = ibooker.book2D("HPTDC Errors", " HPTDC Errors?", 8, -0.5, 8.5, 2, -0.5, 1.5);
